...
void MGPIO_voidSetPortSpecificValue(u8 Copy_u8Port, u16 Copy_u16PortValue)
{
	 switch (Copy_u8Port)
	    {
	    /* Set the  bit corresponding to the intended pin in the ODR register to be as the desired value */
	    case GPIOA:
	    	SET_BITS_REG(GPIOA_REG->GPIOx_ODR, Copy_u16PortValue);
	        break;

	    case GPIOB:
	    	SET_BITS_REG(GPIOB_REG->GPIOx_ODR, Copy_u16PortValue);
	        break;

	    case GPIOC:
	    	SET_BITS_REG(GPIOC_REG->GPIOx_ODR, Copy_u16PortValue);
	        break;
