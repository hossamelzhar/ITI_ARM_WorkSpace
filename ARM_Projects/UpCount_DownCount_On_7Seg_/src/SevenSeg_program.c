...
void SevenSeg_voidInit(const SevenSeg_t * Copy_SevenSeg)
{
	...
	MGPIO_voidSetPortSpecificValue(Copy_SevenSeg->Port , ~(Copy_SevenSeg->Common_Type));
}
...
void SevenSeg_voidDisplayData(const SevenSeg_t * Copy_SevenSeg, u8 Copy_u8Data)
{
	...
	{
		if(Copy_SevenSeg->A <= PIN6)
		{
			MGPIO_voidSetPortSpecificValue(Copy_SevenSeg->Port , SevenSeg_u8CommonAnodeDecoder(Copy_u8Data));
		}
		else if(Copy_SevenSeg->A > PIN6)
		{
			MGPIO_voidSetPortSpecificValue(Copy_SevenSeg->Port , SevenSeg_u8CommonAnodeDecoder(Copy_u8Data)<<7);
		}
	...
	else if(Copy_SevenSeg->Common_Type == COMMON_CATHODE)
	{
		if(Copy_SevenSeg->A <= PIN6)
		{
			MGPIO_voidSetPortSpecificValue(Copy_SevenSeg->Port , SevenSeg_u8CommonCathodeDecoder(Copy_u8Data));
		}
		else if(Copy_SevenSeg->A > PIN6)
		{
			MGPIO_voidSetPortSpecificValue(Copy_SevenSeg->Port , SevenSeg_u8CommonCathodeDecoder(Copy_u8Data)<<7);
		}
	}
	else{}
...
void SevenSeg_voidOFF(const SevenSeg_t * Copy_SevenSeg)
{
	if(Copy_SevenSeg->Common_Type == COMMON_ANODE)
	{
		if(Copy_SevenSeg->A <= PIN6)
		{
			MGPIO_voidSetPortSpecificValue(Copy_SevenSeg->Port , 0x007F);
		}
		else if(Copy_SevenSeg->A > PIN6)
		{
			MGPIO_voidSetPortSpecificValue(Copy_SevenSeg->Port , 0xFF80);
		}
	}
	else if(Copy_SevenSeg->Common_Type == COMMON_CATHODE)
	{
		if(Copy_SevenSeg->A <= PIN6)
		{
			MGPIO_voidSetPortSpecificValue(Copy_SevenSeg->Port , 0x00);
		}
		else if(Copy_SevenSeg->A > PIN6)
		{
			MGPIO_voidSetPortSpecificValue(Copy_SevenSeg->Port , 0x00);
