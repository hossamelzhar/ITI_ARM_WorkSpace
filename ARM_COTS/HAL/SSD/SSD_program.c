...
void SSD_voidDisplayData(const SSD_t * Copy_SevenSeg, u8 Copy_u8Data)
{
	...
	{
		MGPIO_voidSetPortValue(Copy_SevenSeg->Port , (u16)SSD_u8CommonAnodeDecoder(Copy_u8Data));
	}
	else if(Copy_SevenSeg->Common_Type == COMMON_CATHODE)
	{
		MGPIO_voidSetPortValue(Copy_SevenSeg->Port , (u16)SSD_u8CommonCathodeDecoder(Copy_u8Data));
	}
	else{}
